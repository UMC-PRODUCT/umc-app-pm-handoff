...
assert.equal(Object.keys(files).filter(path => /\/\d{2}_.*\.png$/.test(path)).length, 57)
...
console.log('Access checks passed: correct password, wrong password, tampering, all 57 captures, encrypted-only publish output.')
