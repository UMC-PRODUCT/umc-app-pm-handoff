import { androidMetrics, downloadSources, downloadTotal, downloadShare, pageViews, iosSnapshots, versionCrashes } from '../modules/performance'
import './PerformanceDashboard.css'

function BarChart({ items, maximum, unit, shares = false }) {
  return <ul className="metric-bars">{items.map((item) => <li key={item.label}>
    <div className="metric-bar-label"><span>{item.label}</span><strong>{item.value.toLocaleString('ko-KR')}<small>{unit}</small>{shares && <em>{downloadShare(item.value)}%</em>}</strong></div>
...
export default function PerformanceDashboard() {
  return <section className="performance" aria-labelledby="performance-title">
    ...
    <p className="metric-note">Google Play Console과 App Store Connect의 표시 수치를 정리했습니다. 각 화면의 조회 기간이 달라, 플랫폼별 수치를 합산하거나 같은 기간의 성과로 비교하지 않습니다.</p>
    ...
    <section className="performance-block" aria-labelledby="quality-title">
      ...
      <div className="quality-grid">
        ...
        <figure className="metric-figure crash-record"><figcaption><h4>버전별 충돌 기록</h4><p>09.22 표시 화면 · 충돌 발생 건수</p></figcaption><BarChart items={versionCrashes} maximum={146} unit="건" /><p className="metric-note">03.17 표시 화면은 ‘데이터 부족’입니다. 버전별 이용량과 관찰 기간이 없어 충돌률 또는 개선율로 비교할 수 없습니다. 충돌 건수는 영향을 받은 사용자 수와도 다릅니다.</p><strong className="metric-action">다음 확인 → 영향 사용자·발생 상황·버전별 이용량</strong></figure>
      </div>
    </section>

    <details className="metric-reading-guide"><summary>집계 기준과 아직 확인할 수 없는 것</summary><ul><li>문서 기록 기간은 03.10–09.24이며, Android 유입 화면은 08.27–09.23, iOS 경로별 화면은 03.10–09.20입니다. 마지막 날짜의 차이를 미집계 수치로 채우지 않았습니다.</li><li>기기 수·설치 수·다운로드 수는 고유 사용자 수와 다릅니다. 플랫폼을 합친 MAU나 전체 가입자 수는 이 자료만으로 산출할 수 없습니다.</li><li>원본의 일별 숫자와 범례가 없는 추이는 선그래프로 재구성하지 않았습니다. 정확히 표시된 값만 막대와 표로 옮겼습니다.</li><li>유지율과 충돌은 옵트인 데이터로 전체 사용자를 대표한다고 단정할 수 없습니다.</li><li>사용자 만족도, 리뷰 내용, 문의 건수, 운영 업무 절감 시간은 이번 자료에 없습니다. 아래 인수인계 항목에서 별도로 수집합니다.</li></ul></details>
  </section>
}
